...
public class StepDefForiPhone {
	...
	public void validate_Amazon_homepage_and_take_screenshot_of_the_homepage() throws Throwable {
		...
		Assert.assertEquals("Hello, Akija", apf.getAmazonLoginTest().getText());
	...
	public void sort_the_price_from_high_to_low() throws Throwable {
		...
		Select H2L = new Select(apf.getSortBox());
		H2L.selectByIndex(2);
	...
	public void find_out_the_total_number_of_pages() throws Throwable {
		System.out.println("The Total Page Number is: " + apf.getTtlPageNum().getText());
	...
	public void sort_the_price_from_low_to_high() throws Throwable {
		H2L.selectByIndex(1);
		//L2H=new Select(apf.getSearchBox());
		//L2H.selectByIndex(1);
	...
	public void user_able_to_find_out_the_highest_and_lowest_iphone_x_prices_on_the_first_page() throws Throwable {
		...
		totalIphone= apf.getIphonePrice().size();
		IphonePrice = new ArrayList<Integer>();
		System.out.println(totalIphone);
		for (int i = 0; i < totalIphone; i++) {
			String A = apf.getIphonePrice().get(i).getText().replaceAll(",","");
		
			IphonePrice.add(Integer.parseInt(A));
			
			System.out.println(IphonePrice);
		
		}
		
		maximun = Collections.max(IphonePrice);
		minimun = Collections.min(IphonePrice);
